...
def plot_image(img, label):
    ...
    plt.imshow(img, cmap=plt.cm.binary)  # toon de afbeelding
    plt.xlabel(label)  # zet het label op de x-as
    plt.show()
...
def scale_data(X):
    ...
    # YOUR CODE HERE
    max_value = np.amax(X)  # bepaal de maximale waarde in de matrix
    scaled_X = X / max_value  # deel alle elementen in de matrix door de maximale waarde
...
def conf_matrix(labels, pred):
    ...
    # YOUR CODE HERE
    # confusion matrix
    conf_mtx = tf.math.confusion_matrix(labels, pred)  # confusion matrix
...
def conf_els(conf, labels):
    ...
    # YOUR CODE HERE
    # TP
    tp = np.diagonal(conf)  # true positive
    # FP
    fp = np.sum(conf, axis=0) - tp  # false positive
    # FN
    fn = np.sum(conf, axis=1) - tp  # false negative
    # TN
    tn = np.sum(conf) - tp - fp - fn  # true negative

    # return list of tuples
    return list(zip(labels, tp, fp, fn, tn))  # list of tuples
...
def conf_data(metrics):
    ...
    tp = sum([x[1] for x in metrics])  # true positive
    fp = sum([x[2] for x in metrics])  # false positive
    fn = sum([x[3] for x in metrics])  # false negative
    tn = sum([x[4] for x in metrics])  # true negative
