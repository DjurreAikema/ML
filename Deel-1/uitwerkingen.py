...
def draw_graph(data):
    ...
    # Roteren is nodig omdat we de eerste kolom op de x-as en de tweede kolom op de y-as willen zetten
    data = data.T  # Transpose

    x, y = data[0], data[1]  # De eerste kolom op de x-as, de tweede kolom op de y-as
    plt.scatter(x, y, marker='o', c='b', label='Steden')

    plt.xlabel('Grootte van de steden')
    plt.ylabel('Winst van de vervoerder')
    plt.title('Stadsgrootte vs. Winst')
...
def compute_cost(X, y, theta):
    ...
    # 1
    m = len(y)  # Aantal datapunten

    # 2
    # We berekenen de voorspelling h op basis van de huidige waarden van theta.
    # De matrixvermenigvuldiging X * theta voert de lineaire combinatie uit van de kenmerken in X met behulp van de modelparameters in theta en berekent de voorspellingen (h) voor alle datapunten in de dataset.
    h = X.dot(theta)  # matrixvermenigvuldiging van X en theta

    # 3
    diff = h - y  # verschil tussen de voorspelling en de werkelijke waarden

    # 4
    diff_squared = diff ** 2

    # 5
    J = np.sum(diff_squared) / (2 * m)
...
def gradient_descent(X, y, theta, alpha, num_iters):
    ...
    for _ in range(num_iters):
        # 1. Bepaal de voorspelling voor elk datapunt gegeven de huidige waarden van theta
        # X = (97, 2) theta = (1, 2)
        h = X @ theta.T

        # 2. Bepaal het verschil tussen de voorspelling en de werkelijke waarde
        d = h - y  # h = voorspelling, y = werkeleijke waarde

        # 3. Vermenigvuldig dit verschil met elke waarde in X
        gradient = (X.T @ d) / m

        # 4. Update de parameters van theta met behulp van de learning rate (alpha)
        theta -= gradient.T * alpha  # Bepaald in welke richting de gradient verplaatst wordt

        # 5. Bereken de kosten (kostenfunctie) en voeg deze toe aan de lijst van kosten voor latere analyse
        # d_squared_summed = d.T @ d
        # cost = d_squared_summed / (2 * m)
        # costs.append(cost)

        cost = compute_cost(X, y, theta.T)
        costs.append(cost)
