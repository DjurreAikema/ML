...
def plot_number(nrVector):
    ...
    # Zoals altijd gaan we eerst de data visualiseren. In dit geval betekent dat een vector uit
    # weer transformeren in een 20×20 grayscale plaatje.

    # Omdat je weet dat de vector
    # het i-de plaatje uit de dataset representeert, kun je deze vector eenvoudig weer terug omzetten in een 20×20 matrix en die tekenen.
    # Maak daarbij gebruik van de methode numpy.reshape en van matplotlib.matshow.

    # Transformeer de vector naar een 20x20 matrix
    image_matrix = nrVector.reshape(20, 20, order='F')
...
def get_y_matrix(y, m):
    ...
    # De breedte van de matrix wordt bepaald door het aantal klassen (num_classes).
    # Maak een lege schaarse matrix van m x num_classes.
    y_matrix = csr_matrix((m, num_classes))

    # Loop door de elementen van y en zet de juiste posities naar 1.
    for i in range(m):
        y_matrix[i, y[i] - 1] = 1 # -1 omdat de matrix 0-based is. y[i] is 1-based.
...
def compute_cost(Theta2, Theta3, X, y):
    ...
    # Bereken de kosten met behulp van de log-likelihood-kostfunctie
    print("Dimensie van y_matrix:", y_matrix.shape)
    print("Dimensie van predictions:", predictions.shape)
    cost = -1 / m * np.sum(y_matrix * np.log(predictions) + (1 - y_matrix) * np.log(1 - predictions))
...
def nn_check_gradients(Theta2, Theta3, X, y):
    ...
    for i in range(m):
        # Stap 1: Voer forward-propagation uit om de voorspellingen te berekenen
        a1 = np.insert(X[i], 0, 1, axis=1)
        z2 = a1 @ Theta2.T
        a2 = np.insert(sigmoid(z2), 0, 1, axis=1)
        z3 = a2 @ Theta3.T
        a3 = sigmoid(z3)

        # Stap 2: Bereken fout in de outputlaag
        y_matrix = get_y_matrix(y, m)
        delta3 = a3 - y_matrix[i]

        # Stap 3: Bereken fout in de verborgen laag
        delta2 = delta3 @ Theta3[:, 1:] * sigmoid_gradient(z2)

        # Stap 4: Bereken de gradiënten
        Delta2 += delta2.T @ a1
        Delta3 += delta3.T @ a2
